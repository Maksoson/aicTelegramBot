...
class BotFuncs:
    ...
    def get_day_list(self, message):
        ...
        now_day = str(self.other_funcs.compare_user_data(day_reg))
        now_month = str(self.other_funcs.compare_user_data(self.db_funcs.get_month_reg(message.from_user.id)))
        if len(data) > 0:
            for row in data:
                if counter == 1:
    ...
    def reg_start_time(self, message):
        ...
        if start_time.lower() != 'отмена':
            ...
            if not self.other_funcs.check_inserted_time(start_time):
                self.bot.send_message(message.chat.id, self.error + ' Неверные данные, повтори ввод',
                                      reply_markup=self.other_funcs.get_cancel_button())
                self.bot.register_next_step_handler(message, self.reg_start_time)
                return
    ...
    def end_reg_time(self, message):
        ...
        if end_time.lower() != 'отмена':
            ...
            if not self.other_funcs.check_inserted_time(end_time):
                self.bot.send_message(message.chat.id, self.error + ' Неверные данные, повтори ввод',
                                      reply_markup=self.other_funcs.get_cancel_button())
                self.bot.register_next_step_handler(message, self.reg_start_time)
                return
            ...
            if end_time > start_time:
                ...
                if last_info[0] == 'update':
                    ...
                    last_day = str(self.other_funcs.compare_user_data(last_info[2]))
                    last_month = str(self.other_funcs.compare_user_data(last_info[3]))
                    ...
                    if self.db_funcs.update_timetable(row_id, data_reg):
                        self.bot.send_message(message.chat.id,
                                              ...
                                              ' ' + str(self.other_funcs.compare_user_data(data_reg['day_reg'])) + '.'
                                              + str(self.other_funcs.compare_user_data(data_reg['month_reg'])) + ' '
                                              + self.days_dict[str(self.other_funcs.compare_user_data(data_reg['day_reg']))],
                                              reply_markup=self.other_funcs.get_start_keyboard())
                    else:
    ...
    def send_timetable_news(self, message):
        ...
        if last_info[0] != 'delete':
            ...
            day_reg = str(self.other_funcs.compare_user_data(new_info[0]))
            month_reg = str(self.other_funcs.compare_user_data(new_info[1]))
    ...
    def see_times_list_for(self, message, func_type):
        ...
        if len(data) > 0:
            ...
            for row in data:
                now_month = str(self.other_funcs.compare_user_data(row[3]))
                if last_day != self.other_funcs.compare_user_data(row[2]):
                    last_day = self.other_funcs.compare_user_data(row[2])
                    if last_day != 0:
                        result_list += '\n'
                ...
                if counter == int(delete_time_id):
                    if self.db_funcs.delete_from_timetable(row[0]):
                        ...
                        self.bot.send_message(message.chat.id,
                                              ...
                                              str(self.other_funcs.compare_user_data(row[2])) + "." +
                                              str(self.other_funcs.compare_user_data(row[3])) + " " +
                                              self.days_dict[str(self.other_funcs.compare_user_data(row[2]))],
                                              reply_markup=self.other_funcs.get_start_keyboard())
                        self.send_timetable_news(message)
                        break
                    else:
    ...
    def print_all_times(self, message):
        ...
        if len(data) > 0:
            for row in data:
                now_month = str(self.other_funcs.compare_user_data(row[12]))
                if last_day != self.other_funcs.compare_user_data(row[11]):
                    last_day = self.other_funcs.compare_user_data(row[11])
                    counter = 1
                    if last_day != 0:
                        result_list += '\n'
                    result_list += self.pushpin + " " + str(last_day) + '.' + now_month + '.' \
    ...
    def get_days_data(self):
        ...
        for num in range(now.day, now.day + 14):
            ...
            days_dict[str(self.other_funcs.compare_user_data(day_num))] = self.day_names[now_day_num]
            if now_day_num != 6:
                now_day_num += 1
            else:
                now_day_num = 0
